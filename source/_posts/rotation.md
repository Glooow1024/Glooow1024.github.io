...
**内旋/外旋视角的小结**：万向锁的本质可以从两个视角来理解——

- **内旋**：前序旋转让第 1 个旋转轴与第 3 个旋转轴共线（重合），于是原本独立的两次旋转变成绕同一根轴转动，两个角度只能控制同一个转动，丢失 1 个自由度；
- **外旋**：三根世界轴固定不动，轴本身不会重合，只是先后两次旋转的运动效果等价于绕同一根轴转动（如上文矩阵退化后 $\alpha$ 与 $\gamma$ 仅以 $\alpha+\gamma$ 的组合形式出现），自由度坍缩。
